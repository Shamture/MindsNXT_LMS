...
public class CourseController 
{
			...
			public ResponseEntity<Void> deleteCourse(@PathVariable("id") Integer id) {
				...
				return new ResponseEntity<Void>(HttpStatus.NO_CONTENT);
			     ...
			     @GetMapping(value = "course/courseId/{courseId}")
	               public List<Course> findByCourseId(@PathVariable int courseId) 
				   {

		List<Course> customers = repository.findByCourseId(courseId);
		return customers;
	}
	
	            @GetMapping(value = "course/courseName/{courseName}")
	public List<Course> findByCourseName(@PathVariable String courseName) {

		List<Course> customers = repository.findByCourseName(courseName);
		return customers;
	}
